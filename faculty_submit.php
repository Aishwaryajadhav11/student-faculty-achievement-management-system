...
// 🚨 Make EMPID mandatory
if ($empid === '') {
    exit("Error: Employee ID is required. Please go back and enter it.");
}
...
if (isset($_FILES['certificate']) && $_FILES['certificate']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    $certificatePath = 'uploads/faculty/' . $safeName;
}

// Insert into DB
$stmt = $conn->prepare(
    "INSERT INTO faculty_achievements 
     (name, empid, department, event, details, certificate) 
     VALUES (?, ?, ?, ?, ?, ?)"
);
...
if ($ok) {
    echo "<!doctype html><html><head><meta charset='utf-8'><title>Submission Success</title>
          ...
          <div class='card'>
            <h2>🎉 Faculty achievement submitted successfully!</h2>
            <p>Your record has been saved for EMP ID: <b>" . htmlspecialchars($empid) . "</b></p>
            <p>
              <a class='btn primary' href='faculty_view.php?empid=" . urlencode($empid) . "'>📄 View Achievements</a><br>
